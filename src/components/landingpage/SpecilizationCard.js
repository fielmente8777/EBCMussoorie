...
const SpecilizationCard = ({ cardData, index }) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 ">
            <div className={`overflow-hidden max-md:order-1 ${index === 1 || index === 3 ? "order-2" : ""}`}>
                <Swiper
                    loop={true}
                    autoplay={{
                        delay: 2000,
            ...
            <div className="flex flex-col bg-[#D5D5D5] p-3 md:px-10 md:py-24" >
                ...
                {cardData.button && <button className="justify-center self-start px-8 py-2 mt-6 text-lg font-medium tracking-normal text-white bg-[#29422C] rounded-sm leading-[144%]">
                    Book Now
                </button>}
            </div>
        </div>
